...
Lemma~\ref{lem:2} implies that there is no duality gap, and thus, that $T^*_{\xi,j}(\lambda_1,\lambda_0;\Theta_{0},\Theta'')$ can be found by solving the dual problem of the maximization problem \eqref{eq:bounddual}, which is given in Lemma~\ref{lem:3}. \pb{Is this better? My problem with the original wording is that it isn't really clear what minimization problem you're talking about unless you read the appendix.}
...
By default, we set $n=1,000$, $p=3,000$, $c=0.5$, $s=10$, $b=1$, $\alpha=0.2$, $L=100$ and $\lambda_{\min}=0.05\lambda_{\max}$. In each experiment, one of the factors will vary and the performance will be compared in terms of the total power, which measures the overall power along the path and is defined as
\begin{equation}
    \frac{\sum_{l=1}^L|\bar{\mathcal{A}}_l|}{\sum_{l=1}^L|\bar{\mathcal{A}}^*_l|},
\end{equation}
where $\bar{\mathcal{A}}_l$ is the set of features discarded at $\lambda_l$ and $\bar{\mathcal{A}}^*_l$ is the set of features with 0 coefficients at $\lambda_l$ (the complement of the active set). These experiments reveal how each factor affects the screening power as well as some of the limitations of the proposed screening method.
...
\begin{figure}[ht]
    ...
    \caption{Average proportion of feature discarded along the path by interval length $c$ and maximum number of unique observed times.\label{Fig:interval}}
\end{figure}
...
The semiparametric nature of Cox regression not only increases the dimension of $\Theta_\lambda$, but also gives more freedom to $\Theta''$ in Theorem \ref{thm:1}, which controls the shape of the ellipsoid bound for $\Theta_\lambda$. No assumptions are made concerning the underlying distribution of failure times, giving $\Theta''$ the freedom to go to a direction that leads to a ``bad'' shape. The consequences can be seen in the difference between Lemma \ref{lem:3}, where $\Theta''$ is assumed known, and Theorem \ref{thm:2}, which is based on the worst-case value of $\Theta''$. The impact can be interpreted as substituting a weighted variance whose weights $W$ depends on $\Theta''$ \pb{and are somewhat evenly distributed \textbf{is this accurate? W isn't necessarily evenly distrubuted, is it?}}\comment{It is not very accurate. W is yhat in somesense and is somewhat independent of $x_j$ if $x_j$ is not an important feature. I am not sure if it has an equivalent effect as if it is evenly distributed, but I use this assumption to compute the ratio \eqref{eq:alpharatio} in experiment.}, with a weighted variance that puts half weight each on the two most extreme points. The ratio between these two terms can be expressed as:
\begin{equation}
    \label{eq:alpharatio}
...
\begin{figure}[ht]
    ...
    \caption{Average proportion of feature discarded along the path by shape $\alpha$ and ratio \eqref{eq:alpharatio} \label{Fig:alpha}.}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Average proportion of feature discarded along the path by common factors \label{Fig:coxsim}.}
\end{figure}
...
\begin{tabular}{lllll}
\toprule
Screening method & Original Data & Modified Times & Modified Features & Both Modified \\
\midrule
